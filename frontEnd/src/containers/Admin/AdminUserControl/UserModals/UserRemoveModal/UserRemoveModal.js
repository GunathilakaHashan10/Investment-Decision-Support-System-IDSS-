...
class UserRemoveModal extends Component {
    ...
    handleDelete = () => {
        ...
        axios.post(`${myConstants.SEVER_URL}/admin/delete-user?userId=${this.state.userId}`)
            .then(response => {
                setTimeout(() => {
                    this.setState({
                        isLoading: false,
    ...
    render(){
        ...
        if(isDeleteSuccess === null) {
            deleteContent = (
                <div>
                    <h2 className={modalStyles.delete_bank_head}>{`Remove ${this.props.user.firstName}  ${this.props.user.lastName}`}</h2>
            ...
            deleteContent = (
                <div>
                    <div>
                        <h3>{`${this.props.user.firstName}  ${this.props.user.lastName} removed successfully.`}</h3>
