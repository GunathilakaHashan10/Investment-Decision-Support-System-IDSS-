...
class SlopeCompareGraphModal extends Component {
    state = {
        labels : [],
        chartData: {},
        comapanyList: []
    ...
    options = {
        ...
        title: {
          display: true,
          text: "Slope Comparison Bar Chart"
        },
        scales: {
          yAxes: [{
            ticks: {
    ...
    componentWillMount() {
        if(this.props.company.length === 0) {
            this.setState({comapanyList: this.props.slopeData})
        } else {
        const comapanyListCopy = []
            this.props.company.forEach((value, index) => {
                this.props.slopeData.forEach((company, indexCompany) => {
    ...
    componentDidMount() {
        ...
        this.state.comapanyList.forEach((company, index) => {
            labels.push(company.shareName)
            data.push(company.slope.toFixed(2))
...
export default SlopeCompareGraphModal;
